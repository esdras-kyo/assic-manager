...
// No CI sem credenciais, skipa inteiro. Específico do FakeGateway — skipa
// também quando outro provider está ativo (ex.: PAYMENT_PROVIDER=mercadopago).
const temEnv = Boolean(process.env.DATABASE_URL);
const ehFake = (process.env.PAYMENT_PROVIDER ?? "fake") === "fake";

describe.skipIf(!temEnv || !ehFake)(
  "fluxo de pagamento ponta a ponta (fake)",
  () => {
    it(
